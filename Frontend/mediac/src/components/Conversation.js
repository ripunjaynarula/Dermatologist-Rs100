            ...
            <div className="d-flex justify-content-between align-items-center">
              <div className="d-flex flex-row align-items-center conv w-100">
                ...
                <div className="d-flex flex-column line-height ml-2">
                  {" "}
                  <span className="font-weight-bold">{currentUser.email==chat.doctorEmail?chat.patientUsername:chat.doctorUsername}</span>{" "}
                  <span>{chat.messages.length>0?(chat.messages[chat.messages.length-1].text):('Start Conversation')}</span>{" "}
                  <span className="d-flex flex-row align-items-center s-now">
                    <small className="live">{chat.messages.length>0?(chat.messages[chat.messages.length-1].date):('')}</small>
                  </span>{" "}
                </div>
              </div>{" "}
